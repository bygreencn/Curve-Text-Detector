...
int main()
{
	string model_file = "D:\\Src.Disk\\opensource\\caffe\\Curve-Text-Detector\\models\\ctd\\test_ctd_tloc.prototxt";
	string weights_file = "D:\\Src.Disk\\opensource\\caffe\\Curve-Text-Detector\\output\\ctd_tloc.caffemodel";
	int GPUID = 0;
	...
	for (int i = 0; i < ans.size(); ++i) {
		...
		rectangle(im, cvPoint(ans[i][0], ans[i][1]), cvPoint(ans[i][2] + ans[i][0], ans[i][3] + ans[i][1]), cv::Scalar(0, 0, 255), 1, 1, 0);
		cout << endl;
	}
	imshow("test.jpg", im);
	cv::waitKey(0);
